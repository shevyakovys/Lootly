...
async function settings(){
  ...
  const bookingPrefs='<section class="card stack"><div><h2>Правила онлайн-записи</h2><p class="muted tiny">Свободные окна строятся по фактической длительности услуги у выбранного сотрудника.</p></div><div class="grid grid-3"><label class="field"><span>Сетка календаря, мин</span><input id="bookingInterval" type="number" min="1" max="120" step="1" value="'+Number(o.booking_interval_minutes||15)+'" '+(manager?"":"disabled")+'><small>Только визуальная сетка календаря. На шаг онлайн-записи не влияет.</small><div class="interval-presets">'+intervalPresets.map(v=>'<button type="button" class="interval-preset" data-interval-preset="'+v+'" '+(manager?"":"disabled")+'>'+v+'</button>').join("")+'</div></label><label class="field"><span>Горизонт, дней</span><input id="bookingHorizon" type="number" min="1" max="365" value="'+Number(o.booking_horizon_days||60)+'" '+(manager?"":"disabled")+'><small>Насколько далеко вперёд можно записаться.</small></label><label class="field"><span>Минимум до записи, мин</span><input id="bookingNotice" type="number" min="0" max="10080" step="1" value="'+Number(o.min_booking_notice_minutes||0)+'" '+(manager?"":"disabled")+'><small>Например, 120 = не позднее чем за 2 часа.</small></label></div><div class="notice info"><b>Шаг свободных окон теперь автоматический:</b> сотрудник делает услугу 30 минут → окна идут каждые 30 минут; 1 час → каждый час; 1 ч 20 мин → каждые 80 минут. Индивидуальное время задаётся в «Справочники → Сотрудники → Услуги и время».</div>'+(manager?'<button class="btn brand" id="saveBookingPolicy">Сохранить правила записи</button>':'')+'</section>';
  ...
  document.querySelector("#saveBookingPolicy")?.addEventListener("click",async e=>{
    ...
    if(!Number.isInteger(minutes)||minutes<1||minutes>120)return toast("Сетка календаря должна быть целым числом от 1 до 120 минут","error");
