...
async function dashboard(){
  ...
  const [aRes,apptRes,locRes,svcRes,staffRes,hoursRes,widgetRes]=await Promise.all([
  ...
  const metrics=aRes.data||{},all=apptRes.data||[],today=all.filter(x=>new Date(x.start_at)<end),upcoming=all.filter(x=>new Date(x.start_at)>=new Date()).slice(0,30);
  const checklist=[
  ...
  '<section class="quick" style="margin-top:16px"><a href="#/catalog"><span>Справочники</span><b>Добавить услугу</b></a><a href="#/schedule"><span>Расписание</span><b>Изменить график</b></a><a href="#/widgets"><span>Онлайн-запись</span><b>Настроить виджет</b></a><a href="#/settings"><span>Публикация</span><b>Скопировать ссылку</b></a></section>'+
