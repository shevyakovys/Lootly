...
const icon=(n,cls="")=>{
  const paths={
    ...
    schedule:'<path d="M4 6h16M4 12h16M4 18h10"/><circle cx="18" cy="18" r="2"/>',
    widgets:'<rect x="3" y="3" width="8" height="8" rx="2"/><rect x="13" y="3" width="8" height="5" rx="2"/><rect x="13" y="10" width="8" height="11" rx="2"/><rect x="3" y="13" width="8" height="8" rx="2"/>',
    settings:'<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.88l.06.06-2.83 2.83-.06-.06A1.7 1.7 0 0 0 15 19.4a1.7 1.7 0 0 0-1 .6 1.7 1.7 0 0 0-.4 1.1V21h-4v-.1A1.7 1.7 0 0 0 8.6 19.4a1.7 1.7 0 0 0-1.88.34l-.06.06-2.83-2.83.06-.06A1.7 1.7 0 0 0 4.6 15a1.7 1.7 0 0 0-.6-1 1.7 1.7 0 0 0-1.1-.4H3v-4h.1A1.7 1.7 0 0 0 4.6 8.6a1.7 1.7 0 0 0-.34-1.88l-.06-.06 2.83-2.83.06.06A1.7 1.7 0 0 0 9 4.6a1.7 1.7 0 0 0 1-.6 1.7 1.7 0 0 0 .4-1.1V3h4v.1A1.7 1.7 0 0 0 15.4 4.6a1.7 1.7 0 0 0 1.88-.34l.06-.06 2.83 2.83-.06.06A1.7 1.7 0 0 0 19.4 9c.1.36.3.7.6 1 .3.3.7.4 1.1.4h.1v4h-.1c-.4 0-.8.1-1.1.4-.3.3-.5.6-.6 1z"/>',
    client:'<circle cx="12" cy="8" r="4"/><path d="M4 21a8 8 0 0 1 16 0"/>',
    service:'<circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M20 4 8.1 15.9M14.5 14.5 20 20M8.1 8.1 12 12"/>',
    ...
    external:'<path d="M14 5h5v5M10 14 19 5"/><path d="M19 13v6H5V5h6"/>',
    arrowRight:'<path d="M5 12h14M14 7l5 5-5 5"/>',
    arrowLeft:'<path d="M19 12H5M10 7l-5 5 5 5"/>'
  };
  return '<svg class="ui-icon '+esc(cls)+'" viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false">'+(paths[n]||'<circle cx="12" cy="12" r="3"/>')+'</svg>';
};
...
function emptyState(title,text,action=""){return '<div class="empty"><div class="empty-icon">'+icon("service")+'</div><h3>'+esc(title)+'</h3><p>'+esc(text)+'</p>'+action+'</div>'}
...
async function shell(route,title,content){
  ...
  app.innerHTML='<div class="app-shell route-'+esc(route)+'"><aside class="sidebar"><div class="sidebar-brand"><a class="logo" href="#/dashboard"><span class="brand-mark">L</span><span>Lootly</span></a><span class="sidebar-product">CRM</span></div><nav class="sidebar-nav"><div class="nav-section-label">Рабочее пространство</div>'+
    navLink("dashboard","Обзор","dashboard",route)+navLink("calendar","Календарь","calendar",route)+navLink("catalog","Клиенты и услуги","catalog",route)+navLink("schedule","Команда и график","schedule",route)+
    '<div class="nav-section-label">Каналы и система</div>'+navLink("widgets","Онлайн-запись","widgets",route)+navLink("settings","Настройки","settings",route)+
    '</nav><div class="sidebar-foot"><div class="profile-mini"><span class="profile-avatar">'+initials(o?.name||s?.user?.email||"L")+'</span><div><strong>'+esc(o?.name||s?.user?.email)+'</strong><span>'+esc(p?.role||"")+'</span></div></div><button id="logout" class="sidebar-logout">'+icon("external")+'<span>Выйти</span></button></div></aside>'+
    '<div class="main"><div class="mobile-top"><a class="logo" href="#/dashboard"><span class="brand-mark">L</span>Lootly</a><span class="muted tiny">'+esc(o?.name||"")+'</span></div><header class="topbar"><div class="topbar-title"><span class="breadcrumb">Рабочее пространство</span><strong>'+esc(title)+'</strong></div><div class="top-actions"><a class="topbar-booking-link" target="_blank" href="#/book/'+esc(o?.slug||"")+'">'+icon("external")+'<span>Онлайн-запись</span></a><div class="topbar-profile"><span class="topbar-avatar">'+initials(o?.name||s?.user?.email||"L")+'</span><div><b>'+esc(o?.name||"Lootly")+'</b><span>'+esc(p?.role||"")+'</span></div></div></div></header><main class="page">'+content+'</main>'+
    '<nav class="mobile-nav"><a class="'+(route==="dashboard"?"active":"")+'" href="#/dashboard">'+icon("dashboard")+'<span>Обзор</span></a><a class="'+(route==="calendar"?"active":"")+'" href="#/calendar">'+icon("calendar")+'<span>Календарь</span></a><a class="'+(route==="catalog"?"active":"")+'" href="#/catalog">'+icon("catalog")+'<span>Справочники</span></a><a class="'+(route==="widgets"?"active":"")+'" href="#/widgets">'+icon("widgets")+'<span>Виджеты</span></a><a class="'+(route==="settings"?"active":"")+'" href="#/settings">'+icon("settings")+'<span>Ещё</span></a></nav></div></div>';
...
async function dashboard(){
  ...
  const onboarding=done<checklist.length?'<section class="card onboarding"><div class="spread"><div><h2>Настройка '+done+'/'+checklist.length+'</h2><p>Ещё немного — и клиенты смогут записываться сами.</p></div><span style="font-size:34px;font-weight:850">'+Math.round(done/checklist.length*100)+'%</span></div><div class="checklist">'+checklist.map(x=>'<a href="'+x[2]+'" class="checkitem '+(x[0]?"done":"")+'"><span class="checkdot">'+(x[0]?icon("check"):'<span class="check-pending"></span>')+'</span><span>'+esc(x[1])+'</span></a>').join("")+'</div></section>':"";
  const rows=upcoming.map(x=>appointmentRow(x)).join("");
  const content='<div class="page-head dashboard-page-head"><div><span class="eyebrow">CRM Dashboard</span><h1>Добро пожаловать в '+esc(o.name)+'</h1><p>Ключевые показатели и ближайшие действия · '+new Intl.DateTimeFormat("ru-RU",{timeZone:defaultTz,weekday:"long",day:"numeric",month:"long"}).format(now)+'</p></div><div class="cluster"><a class="btn secondary" href="#/widgets">Поделиться записью</a><button class="btn brand" id="focusNewBooking">'+icon("plus")+'<span>Новая запись</span></button></div></div>'+
...
function appointmentRow(x){return '<tr><td><b class="appt-time">'+dtZone(x.start_at,x.locations?.timezone)+'</b></td><td><div class="appt-branch"><span class="meta-icon meta-icon-building" aria-hidden="true"></span><span>'+esc(x.locations?.name||"—")+'</span></div></td><td><div class="person"><span class="avatar">'+initials(x.customers?.name)+'</span><div><a href="#/client/'+x.customer_id+'"><b>'+esc(x.customers?.name||"Клиент")+'</b></a></div></div></td><td><span class="phone-cell">'+esc(x.customers?.phone||"—")+'</span></td><td><span class="service-name">'+esc(x.services?.name||"—")+'</span><span class="duration-chip">'+durationLabel(x.duration_minutes)+'</span></td><td><div class="person compact-person"><span class="avatar staff-avatar">'+initials(x.staff_members?.name)+'</span><div><b>'+esc(x.staff_members?.name||"—")+'</b></div></div></td><td><span class="status '+esc(x.status)+'">'+esc(statusLabel(x.status))+'</span></td><td><div class="cluster row-actions">'+(["booked","confirmed"].includes(x.status)?'<button class="btn secondary sm icon-action" title="Подтвердить" aria-label="Подтвердить" data-status="'+x.id+'" data-to="confirmed">'+icon("check")+'</button><button class="btn secondary sm" data-status="'+x.id+'" data-to="completed">Готово</button><button class="btn danger sm icon-action" title="Отменить" aria-label="Отменить" data-status="'+x.id+'" data-to="canceled">'+icon("close")+'</button>':"")+'</div></td></tr>'}
...
async function renderQuickBooking(){
  ...
  const updateManualHint=()=>{
    ...
    if(!hint)return;
    const staffRow=eligibleStaff.find(x=>x.id===staffId),service=svc.find(x=>x.id===f.elements.service.value);
    const duration=Math.max(1,Number(staffRow?.duration_minutes||service?.duration_minutes||0));
    if(!time){hint.textContent="Длительность: "+durationLabel(duration)+". Укажите время начала.";return}
    const [h,m]=time.split(":").map(Number),total=h*60+m+duration,endH=Math.floor(total/60)%24,endM=total%60,dayShift=Math.floor(total/1440);
    hint.textContent="Окончание: "+String(endH).padStart(2,"0")+":"+String(endM).padStart(2,"0")+" · "+durationLabel(duration)+(dayShift?" · следующий день":"");
  };
  ...
  const loadStaff=async({resetDay=false}={})=>{
    ...
    try{
      eligibleStaff=await rpcRetry("get_admin_staff_for_service",{p_location_id:location.id,p_service_id:f.elements.service.value},{attempts:2,timeout:8000})||[];
...
async function catalog(){
  ...
  const render=tab=>{
    ...
    const renderRows=()=>{
      ...
      document.querySelector("#entityList").innerHTML=rows.length?rows.map(x=>entityCard(tab,x,manager,data)).join(""):emptyState("Ничего не найдено","Измените поиск или фильтр.");
    };
    const cards=data[tab].map(x=>entityCard(tab,x,manager,data)).join("");
    ...
    if(manager){
      ...
      if(tab==="staff"){
        ...
        document.querySelector("#entityList")?.addEventListener("click",e=>{
          const button=e.target.closest("[data-configure-staff]");if(!button)return;
          assignmentForm.elements.staff.value=button.dataset.configureStaff;
          syncAssignment();
          assignmentForm.scrollIntoView({behavior:"smooth",block:"start"});
        });
        document.querySelectorAll("[data-duration-preset]").forEach(button=>button.addEventListener("click",()=>{
          assignmentForm.elements.duration_mode.value="custom";
          const total=Number(button.dataset.durationPreset||0);
          assignmentForm.elements.duration_hours.value=String(Math.floor(total/60));
          assignmentForm.elements.duration_minutes_part.value=String(total%60);
          refreshDurationDraft();
        }));
...
function entityCard(tab,x,editable=false,data=null){
  ...
  if(tab==="staff"){
    const avatar=x.avatar_url?'<img class="staff-photo" src="'+esc(x.avatar_url)+'" alt="" loading="lazy">':'<span class="avatar">'+initials(x.name)+'</span>';
    const assignments=(data?.assignments||[]).filter(a=>a.staff_id===x.id);
    const services=assignments.map(a=>{
      const svc=(data?.services||[]).find(s=>s.id===a.service_id);if(!svc)return"";
      const effective=Number(a.duration_override_minutes||svc.duration_minutes||0);
      const custom=Number(a.duration_override_minutes||0)>0;
      return '<span class="staff-service-chip"><b>'+esc(svc.name)+'</b><span>'+durationLabel(effective)+(custom?' · индивидуально':'')+'</span></span>';
    }).join("");
    return '<div class="staff-list-row"><div class="staff-list-main"><div class="person">'+avatar+'<div><b>'+esc(x.name)+'</b><small>'+(x.active?"Активен":"Неактивен")+' · '+assignments.length+' услуг</small></div></div><div class="staff-service-chips">'+(services||'<span class="muted tiny">Услуги не назначены</span>')+'</div></div>'+(editable?'<button type="button" class="btn secondary sm" data-configure-staff="'+x.id+'">'+icon("service")+'<span>Услуги и время</span></button>':"")+'</div>'
  }
...
async function widgets(){
  ...
  const content='<div class="page-head"><div><h1>Виджеты</h1><p>Настройте сценарий записи, оформление и способ установки на сайт.</p></div><div class="cluster"><span id="widgetSaveState" class="save-state saved">Сохранено</span><button class="btn brand" id="newWidget">'+icon("plus")+'<span>Новый виджет</span></button></div></div>'+
...
async function settings(){
  ...
  const bookingPrefs='<section class="card stack"><div><h2>Правила онлайн-записи</h2><p class="muted tiny">Свободные окна строятся по фактической длительности услуги у выбранного сотрудника.</p></div><div class="grid grid-2"><label class="field"><span>Горизонт, дней</span><input id="bookingHorizon" type="number" min="1" max="365" value="'+Number(o.booking_horizon_days||60)+'" '+(manager?"":"disabled")+'><small>Насколько далеко вперёд можно записаться.</small></label><label class="field"><span>Минимум до записи, мин</span><input id="bookingNotice" type="number" min="0" max="10080" step="1" value="'+Number(o.min_booking_notice_minutes||0)+'" '+(manager?"":"disabled")+'><small>Например, 120 = не позднее чем за 2 часа.</small></label></div><div class="notice info"><b>Интервал свободных окон рассчитывается автоматически:</b> при длительности 30 минут окна идут каждые 30 минут; при 1 часе — каждый час; при 1 ч 20 мин — каждые 80 минут. Индивидуальное время задаётся в «Справочники → Сотрудники → Услуги и время». Визуальная сетка меняется отдельно на экране «Календарь».</div>'+(manager?'<button class="btn brand" id="saveBookingPolicy">Сохранить правила записи</button>':'')+'</section>';
  ...
  document.querySelector("#saveBookingPolicy")?.addEventListener("click",async e=>{
    const btn=e.currentTarget,minutes=Number(o.booking_interval_minutes||15),horizon=Number(document.querySelector("#bookingHorizon").value),notice=Number(document.querySelector("#bookingNotice").value);
...
async function bookingExperience({widgetKey=null,slug=null}){
  ...
  const cfg=widgetKey?data.widget:{title:data.organization.name,subtitle:"Онлайн-запись",primary_color:"#206bc4",show_staff_step:true,allow_any_staff:true,show_branding:true,waitlist_enabled:false,step_order:["location","service","staff","datetime"]};
  ...
  const slotRange=x=>x?slotTime(x.start_at)+"–"+slotTime(x.end_at):"";
  ...
  async function render(){
    ...
    if(step==="staff"){
      ...
      else body='<div class="option-grid">'+(cfg.allow_any_staff?'<button type="button" class="option '+(!state.staff?"active":"")+'" data-staff=""><div class="person"><span class="avatar any-staff-avatar">'+icon("staff")+'</span><div><b>Любой специалист</b><small>Подберём ближайшее время</small></div></div></button>':"")+state.staffList.map(x=>'<button type="button" class="option '+(state.staff?.id===x.id?"active":"")+'" data-staff="'+x.id+'"><div class="person">'+staffVisual(x)+'<div><b>'+esc(x.name)+'</b><small>Специалист · '+durationLabel(x.duration_minutes||state.service?.duration_minutes)+'</small></div></div></button>').join("")+'</div>';
    }
    if(step==="datetime"){
      let slotsPart="";
      if(state.loadError)slotsPart='<div class="notice error">'+esc(state.loadError)+'</div><button type="button" class="btn secondary" id="retrySlots">Повторить загрузку</button>';
        ...
        slotsPart=groups.map(([title,test])=>{const items=state.slots.map((x,i)=>({x,i})).filter(({x})=>test(x));return items.length?'<div class="slot-section"><div class="slot-section-title">'+title+'</div><div class="slot-grid">'+items.map(({x,i})=>'<button type="button" class="slot '+(state.slot===i?"active":"")+'" data-slot="'+i+'"><b>'+slotTime(x.start_at)+'</b><span>'+slotTime(x.end_at)+'</span></button>').join("")+'</div></div>':""}).join("");
      }
      ...
      body='<div class="booking-date-nav"><button type="button" class="btn ghost sm" id="datePrev" '+(state.dateOffset===0?"disabled":"")+'>'+icon("arrowLeft")+'</button><b>'+windowLabel+'</b><button type="button" class="btn ghost sm" id="dateNext" '+(state.dateOffset+7>bookingHorizon?"disabled":"")+'>'+icon("arrowRight")+'</button></div><div class="date-strip">'+dateChips()+'</div>'+slotsPart;
    }
    if(step==="contact"){
      const chosen=state.slots[state.slot];
      body='<form id="contactForm" class="stack"><div class="summary"><b>'+esc(state.service?.name||"")+'</b><div class="muted tiny" style="margin-top:4px">'+esc(state.location?.name||"")+' · '+(chosen?slotDateTime(chosen.start_at)+"–"+slotTime(chosen.end_at):"")+' · '+durationLabel(chosen?Math.max(1,Math.round((new Date(chosen.end_at)-new Date(chosen.start_at))/60000)):(state.staff?.duration_minutes||state.service?.duration_minutes||0))+(state.staff?" · "+esc(state.staff.name):" · специалист будет назначен автоматически")+'</div></div><label class="field"><span>Ваше имя</span><input name="name" autocomplete="name" value="'+esc(state.contact.name)+'" required><span class="field-error" data-error="name"></span></label><label class="field"><span>Телефон</span><input name="phone" type="tel" inputmode="tel" autocomplete="tel" value="'+esc(state.contact.phone)+'" placeholder="+7 999 000-00-00" required><span class="field-error" data-error="phone"></span></label><label class="field"><span>Email <span class="muted">(необязательно)</span></span><input name="email" type="email" autocomplete="email" value="'+esc(state.contact.email)+'"><span class="field-error" data-error="email"></span></label><div class="contact-actions"><button type="button" class="btn secondary" id="contactBack">Назад</button><button class="btn brand" id="bookSubmit" style="background:'+cfg.primary_color+'">Подтвердить запись</button></div><div id="bookResult"></div></form>';
    }
