...
async def seed(session: AsyncSession) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID, uuid.UUID, uuid.UUID]:
    ...
    session.add(Location(id=location_id, organization_id=org_id, name="Main", timezone="Europe/Moscow"))
    session.add(StaffMember(id=staff_id, organization_id=org_id, location_id=location_id, name="Anna"))
    session.add(Service(id=service_id, organization_id=org_id, name="Haircut", duration_minutes=60, price=Decimal("2000")))
    session.add(Customer(id=customer_id, organization_id=org_id, name="Ivan", phone="+70000000000"))
    await session.flush()
...
async def test_overlapping_active_appointment_is_rejected(session: AsyncSession) -> None:
    ...
    with pytest.raises(AppointmentConflict):
        await service.create(first.model_copy(update={"start_at": datetime(2026, 10, 1, 10, 30, tzinfo=UTC)}))
